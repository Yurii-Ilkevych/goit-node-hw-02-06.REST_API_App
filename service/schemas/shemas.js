...
const schemaFullContact = Joi.object({
  ...
  email: Joi.string().required()
    .email({ tlds: { allow: ["com", "net", "ua", "ca", "uk"] } }).pattern(regexMail)
    .required(),
...
const userSchemaValid = Joi.object({
  email: Joi.string().email({ tlds: { allow: ["com", "net", "ua", "ca", "uk"] } }).pattern(regexMail).required(),
  password: Joi.string().required().min(5).max(25).regex(/^(?=.*[a-zA-Z])(?=.*\d)/)
})
